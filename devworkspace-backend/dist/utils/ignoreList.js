...
function segmentsMatch(pattern, candidate) {
...
function segmentsEqual(pattern, candidate) {
...
export function compileIgnoreRules(defaultPatterns = DEFAULT_IGNORES, gitignoreContents = []) {
    const rules = [];
    ...
    for (const content of gitignoreContents) {
        for (const rawLine of content.split(/\r?\n/)) {
            ...
            if (!line || line.startsWith("#"))
                continue;
            if (line.startsWith("!") && line.slice(1).trim().startsWith("#"))
                continue;
...
export async function collectGitignores(root) {
    const contents = [];
    const seen = new Set();
    async function walk(dir, depth) {
        if (depth > 6 || contents.length > 50)
            return;
        let dirents;
        try {
