...
import { PathGuardError, resolveInWorkspace, } from "../utils/pathGuard.js";
import { collectGitignores, compileIgnoreRules, } from "../utils/ignoreList.js";
...
export const MAX_TREE_TOTAL = 50000;
...
function isValidName(name) {
    ...
    if (name === "." || name === "..")
        return false;
    if (/[<>:"|?*\\]/u.test(name))
        return false; // illegal on Windows
    return true;
}
export class FileService {
    async getTree(workspacePath) {
     ...
     */
    async listDirs(workspacePath, dirPath) {
    ...
    async readFile(workspacePath, filePath) {
        ...
        if (stat.size > MAX_FILE_SIZE) {
            throw new PathGuardError(`File exceeds the ${(MAX_FILE_SIZE / 1024 / 1024).toFixed(0)} MB limit`, 413);
        }
    ...
    async writeFile(workspacePath, filePath, content) {
        ...
        if (content.length > MAX_FILE_SIZE) {
            throw new PathGuardError(`Write exceeds the ${(MAX_FILE_SIZE / 1024 / 1024).toFixed(0)} MB limit`, 413);
        }
    ...
    async createItem(workspacePath, parentPath, name, kind) {
        if (!isValidName(name)) {
    ...
    async deleteItem(workspacePath, filePath) {
    ...
    async buildTree(absoluteRoot, rules) {
        let total = 0;
        ...
        const walk = async (dir, depth) => {
            ...
            for (const dirent of dirents) {
                ...
                if (dirent.isDirectory()) {
                    ...
                    if (rules.isIgnored(childRelative, false))
                        continue;
                    let stat;
                    try {
